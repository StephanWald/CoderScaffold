...
- [x] **CFG-01**: A committed `.env.example` documents every configuration variable with safe placeholder values
...
- [x] **OPS-01**: README documents the external reverse-proxy contract: HTTP `:7080`, wildcard TLS cert for `*.<apps-domain>`, preserved `Host` + WebSocket upgrade headers, no terminal-breaking buffering
- [x] **OPS-02**: README documents first-admin bootstrap and the start-order caveat (DB-first / create-admin / start server)
- [x] **OPS-03**: README documents the `chown 999:999` prerequisite and bring-up sequence
...
*Last updated: 2026-06-17 at v1.0 milestone close — corrected stale CFG-01/OPS-01/OPS-02/OPS-03 traceability (delivered in Phase 1 Plan 02; table had lagged). All 22 v1 requirements complete.*
